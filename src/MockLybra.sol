...
contract MockLybra is EUSD, Governable {
    ...
    uint256 year = 86400 * 365; // secs in a year
    ...
    constructor(address stETHAddress, address priceFeedAddress) {
        gov = msg.sender;
        lido = Ilido(stETHAddress);
     ...
     */
    function excessIncomeDistribution(uint256 payAmountInEusd) external {
        uint256 payoutEther = (payAmountInEusd * 1e18) / _etherPrice();
        require(
            payoutEther <=
        ...
        if (payAmountInEusd > income) {
            // _transfer(msg.sender, address(serviceFeePool), income);
            _transfer(msg.sender, gov, payAmountInEusd);
            // serviceFeePool.notifyRewardAmount(income);

            uint256 sharesAmount = getSharesByMintedEUSD(payAmountInEusd - income);
            emit Log("shares amount", sharesAmount);
            if (sharesAmount == 0) {
                //EUSD totalSupply is 0: assume that shares correspond to EUSD 1-to-1
                sharesAmount = payAmountInEusd - income;
            }
            ...
            feeStored = income - payAmountInEusd;
            // emit FeeDistribution(
            //     address(serviceFeePool),
            //     payAmount,
            //     block.timestamp
        ...
        emit LSDistribution(payoutEther, payAmountInEusd, block.timestamp);
    }
