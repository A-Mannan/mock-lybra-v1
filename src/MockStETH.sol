 ...
 * `_getTotalPooledEth` function. `Lybra.sol` contract inherits stETH and defines
 * the `_getTotalPooledEth` function.
 *
 ...
 * which equals to:
 *
 *   shares[account] * _getTotalPooledEth() / _getTotalShares()
 *
 * For example, assume that we have:
 *
 *   _getTotalPooledEth() -> 1000 stETH
 *   sharesOf(user1) -> 100
 *   sharesOf(user2) -> 400
 *
 * Therefore:
 *
 *   balanceOf(user1) -> 2 tokens which corresponds 200 stETH
 *   balanceOf(user2) -> 8 tokens which corresponds 800 stETH
 *
 * Since balances of all token holders change when the amount of total supplied stETH
...
contract MockStETH is IERC20 {
    ...
    function submit(address) external payable returns (uint256) {
        ...
        totalEther = _getTotalPooledEth() + msg.value;
        updateTime = block.timestamp;
        return sharesAmount;
    }
    ...
    /**
     * @return the amount of stETH in existence.
     *
     * @dev Always equals to `_getTotalPooledEth()` since token amount
     * is pegged to the total amount of stETH controlled by the protocol.
     */
    function totalSupply() public view returns (uint256) {
        return _getTotalPooledEth();
    }
    ...
    /**
     * @return the amount of shares that corresponds to `_stETHAmount` protocol-supplied stETH.
     */
    function getSharesByPooledEth(
        uint256 _stETHAmount
    ) public view returns (uint256) {
        uint256 totalPooledEth = _getTotalPooledEth();
        if (totalPooledEth == 0) {
            return 0;
        } else {
            return _stETHAmount.mul(_getTotalShares()).div(totalPooledEth);
        }
    }

    /**
     * @return the amount of stETH that corresponds to `_sharesAmount` token shares.
     */
    function getPooledEthByShares(
        ...
        if (totalShares == 0) {
            ...
            return
                _sharesAmount.mul(_getTotalPooledEth()).div(totalSharesAmount);
        }
    }
